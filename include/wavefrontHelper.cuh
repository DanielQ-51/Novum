...
struct RayQueue
{
    ...
    __device__ __forceinline__ void getPackedState(int idx, int& pixelIdx, int& depth) const {
        uint32_t state = __ldcs(&payload[idx]).y;
        pixelIdx = state & 0x03FFFFFF;
        depth    = state >> 26;
    }

    __device__ __forceinline__ uint32_t getStackRaw(int idx) const {
        return __ldcs(&payload[idx]).z;
    }

    __device__ __forceinline__ float getLastPDF(int idx) const {
        return __uint_as_float(__ldcs(&payload[idx]).w);
    }
};
...
static __device__ __forceinline__ int getCurrentMediumID(uint32_t stack) {
    return (int)(stack & 0xF);
}

static __device__ __forceinline__ void pushMedium(uint32_t& stack, int mediumID) {
    stack = (stack << 4) | (mediumID & 0xF);
}

static __device__ __forceinline__ void popMedium(uint32_t& stack) {
    stack >>= 4;
}
...
struct ShadowQueue {
    ...
    __device__ __forceinline__ void setIgnore (
        ...
        __stcs(&origin_plus_dist[idx].w, -1.0f);
    }
...
__device__ __forceinline__ uint32_t expandBits2D(uint32_t v) {
    v = (v | (v << 8)) & 0x00FF00FFu;
    v = (v | (v << 4)) & 0x0F0F0F0Fu;
    v = (v | (v << 2)) & 0x33333333u;
    v = (v | (v << 1)) & 0x55555555u;
...
__device__ __forceinline__ uint32_t getMorton18_2D(uint32_t u, uint32_t v) {
...
__device__ __forceinline__ uint32_t getMorton12_3D(uint32_t x, uint32_t y, uint32_t z) {
...
__device__ __forceinline__ uint32_t generateMortonSortKey(float4 origin, float4 dir, float4 sceneMin, float invDiameter) {
    ...
    uint32_t u = (uint32_t)fminf(fmaxf((res.x * 0.5f + 0.5f) * 511.0f, 0.0f), 511.0f);
    uint32_t v = (uint32_t)fminf(fmaxf((res.y * 0.5f + 0.5f) * 511.0f, 0.0f), 511.0f);
    uint32_t dirKey = getMorton18_2D(u, v); // 18 bits total

    uint32_t px = (uint32_t)fminf(fmaxf((origin.x - sceneMin.x) * invDiameter * 15.0f, 0.0f), 15.0f);
    uint32_t py = (uint32_t)fminf(fmaxf((origin.y - sceneMin.y) * invDiameter * 15.0f, 0.0f), 15.0f);
    uint32_t pz = (uint32_t)fminf(fmaxf((origin.z - sceneMin.z) * invDiameter * 15.0f, 0.0f), 15.0f);
    uint32_t posKey = getMorton12_3D(px, py, pz); // 12 bits total

    return (posKey << 18) | dirKey;
}

__device__ __forceinline__ uint32_t generateMaterialSortKey(int materialID, int textureIndex) {
    uint32_t mat16 = ((uint32_t)materialID) & 0xFFFFu;
    uint32_t tex16 = ((uint32_t)textureIndex) & 0xFFFFu;
