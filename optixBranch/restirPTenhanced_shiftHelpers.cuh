    ...
    // First, scale RR for x_k-1. This can be done right away since we do RR before the throughput scaling at each vertex.
{
    ...
    float4 xkminus1_outDirLocal = toLocal(f4(xkminus1_to_xk_direction_normalized), f4(xkminus1_normal));

    float4 xkminus1_f_val;
    float xkminus1_pdf;
    f_pdf_eval(
    ...
    throughput *= f3(xkminus1_f_val) * xkminus1_outgoing_cosine / xkminus1_pdf;
    ...
    float4 rc_xk_inDirLocal = toLocal(f4(xkminus1_to_xk_direction_normalized), f4(rc_xk_normal));
    float4 rc_xk_outDirLocal = toLocal(f4(rcWi), f4(rc_xk_normal));
    ...
    throughput *= f3(rc_xk_f_val) * rc_xk_outgoing_cosine / rc_xk_p_sampled_light;
    ...
    // Next, scale RR for x_k
{
    ...
    throughput *= f3(rc_xk_f_val) * rc_xk_outgoing_cosine / rc_xk_bsdf_pdf;
...
__device__ __forceinline__ inline ShiftResult perform_K_is_D_reconnection(
    ...
    // Now throughput is updated to that of entering x_k/leaving x_k-1
    throughput *= f3(xkminus1_f_val) * xkminus1_outgoing_cosine / p_sampled_light;
